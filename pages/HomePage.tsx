...
// Fix: Removed MOCK_MATCHES and imported GOOGLE_SHEET_MATCHES_TAB for dynamic data fetching.
  ...
  useEffect(() => {
    const loadUpcomingMatch = async () => {
      try {
        setIsLoading(true);
        setError(null);
        // Fix: Fetch matches dynamically instead of using MOCK_MATCHES
      ...
      {!isLoading && !error && !upcomingMatch && (
       ...
       <section className="text-center py-12 bg-white rounded-lg shadow-lg">
          ...
          <div className="flex justify-center space-x-4">
            ...
            <a href="#/club/about" className={`px-6 py-3 bg-[${ACCENT_COLOR}] text-[${THEME_WHITE}] font-semibold rounded-lg shadow hover:bg-[${ACCENT_COLOR}]/90 transition-colors`}>Meet the Team</a>
          </div>
        </section>
    </div>
  );
