...
// IMPORTANT: Use the SAME URL from your Google Apps Script deployment as the Sponsor page.
...
const ContactPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (result.status === 'success') {
                setSuccessMessage("Thank you for your message! We've received it and will get back to you as soon as possible.");
                form.reset();
            } else {
            ...
            setError(`Sorry, there was an issue sending your message. Please try again. (Details: ${err instanceof Error ? err.message : String(err)})`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="bg-surface border border-border p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
            <h1 className="text-4xl font-bold text-primary mb-2 pb-3 border-b-2 border-border">Contact Us</h1>
            <p className="text-gray-600 mb-8">Have a question or just want to say hi? Drop us a line!</p>
            
            <form onSubmit={handleSubmit} className="space-y-6">
                <input type="hidden" name="formType" value="contact" />
                <div>
                    <label htmlFor="name" className="block text-sm font-medium text-gray-700">Your Name</label>
                    <input type="text" id="name" name="name" required disabled={isLoading} className="mt-1 block w-full bg-background border-border rounded-md shadow-sm py-2 px-3 text-accent focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm disabled:opacity-50" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="subject" name="subject" required disabled={isLoading} className="mt-1 block w-full bg-background border-border rounded-md shadow-sm py-2 px-3 text-accent focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm disabled:opacity-50" />
                </div>
                <div>
                    <label htmlFor="message" className="block text-sm font-medium text-gray-700">Message</label>
                    <textarea id="message" name="message" rows={5} required disabled={isLoading} className="mt-1 block w-full bg-background border-border rounded-md shadow-sm py-2 px-3 text-accent focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm disabled:opacity-50"></textarea>
                </div>
